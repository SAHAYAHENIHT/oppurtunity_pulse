...
function App() {
  ...
  useEffect(() => {
    if (profile.skills && opportunities.length > 0) {
      const fetchMatches = async () => {
        try {
          const res = await fetch('http://localhost:5000/api/opportunities/matched', {
            method: 'POST',
        ...
        } catch (err) { console.error('Match sync failed:', err); }
      };
      
      const timer = setTimeout(fetchMatches, 1500);
      return () => clearTimeout(timer);
    }
  }, [profile.skills, opportunities.length]);
  ...
  const handleFileChange = (e, type) => {
    ...
    reader.onloadend = async () => {
      ...
      setProfile({ ...profile, [type === 'pic' ? 'profilePicture' : 'resumeUrl']: result })
      
      // If resume is uploaded, extract skills and find matched jobs
      if (type === 'resume') {
        setIsExtractingSkills(true)
        try {
          // HIT THE REAL BACKEND PARSER
          const parseRes = await fetch('http://localhost:5000/api/resume/parse', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ fileBase64: result })
          })
          ...
          if (parseData.success) {
            ...
            const merged = Array.from(new Set([...existingSkills, ...parseData.skills])).join(', ')
            
            // Populate BOTH skills and the raw text for the user to see
            setProfile(prev => ({ 
              ...prev, 
              skills: merged,
              resumeText: parseData.text // Support raw text display
            }))
            
            // Find matched jobs based on THESE skills
            const matchRes = await fetch('http://localhost:5000/api/opportunities/matched', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ skills: parseData.skills, opportunities })
            })
            const matchData = await matchRes.json()
            if (matchData.success) {
              setMatchedJobs(matchData.matched)
              setAlertMsg({ type: 'success', text: `✨ Automation Success: Extracted ${parseData.skills.length} skills from your PDF!` })
            }
          }
        } catch (err) { console.error(err) }
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="sidebar-slim">
        ...
        <div className={`sb-nav-item ${activeTab === 'matched' ? 'active' : ''}`} onClick={() => setActiveTab('matched')}>
          <Star size={24} />
            ...
            <>
              ...
              <div className="feed-section">
                ...
                <div className="opportunities-grid">
                  {opportunities
                    .filter(opp => {
                      const matchesSearch = (opp.title + opp.organization).toLowerCase().includes(searchQuery.toLowerCase())
                      ...
                      return matchesSearch && matchesCategory && matchesLocation && matchesDeadline && matchesStudent && matchesFresher && matchesCash
                    })
                    .map(opp => (
             ...
             <div className="feed-section">
                <div className="section-header">
                  <div className="section-line"></div>
                  <h2>Resume Matched Jobs</h2>
                </div>
                
                {matchedJobs.length > 0 ? (
                  <div className="opportunities-grid">
                    <div style={{gridColumn: '1/-1', marginBottom: '1.5rem', background: '#ecfdf5', padding: '1rem', borderRadius: '12px', color: '#065f46', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '10px'}}>
                      <Star size={20} fill="#059669" /> These jobs were shortlisted based on your extracted resume skills.
                    </div>
                    {matchedJobs.map(opp => (
                      <div key={opp.id} className="opp-card" style={{ border: '2px solid #10b98144' }}>
                        <div className="card-org-row">
                          <div className="org-logo">{opp.organization[0]}</div>
                          <div className="match-badge-group">
                            {opp.resumeMatchedSkills?.map(skill => (
                              <span key={skill} className="skill-match-tag">✨ {skill}</span>
                            ))}
                          </div>
                        </div>
                        <h3 className="card-title">{opp.title}</h3>
                        <p className="card-org">{opp.organization} • {opp.location}</p>
                  ...
                  <div style={{textAlign: 'center', padding: '4rem 2rem', background: 'white', borderRadius: '24px', border: '1px solid var(--border)'}}>
                    <Star size={64} color="#e2e8f0" style={{marginBottom: '1rem'}} />
                    <h3>No Matched Jobs Yet</h3>
                    <p style={{color: 'var(--text-muted)', maxWidth: '400px', margin: '1rem auto'}}>
                      Upload your resume in the <b>Profile</b> section. We'll extract your skills and shortlist the best opportunities for you!
                    </p>
                    <button className="btn-login" onClick={() => setIsEditingProfile(true)}>Go to Profile</button>
                  </div>
                )}
             </div>
          ) : activeTab === 'analytics' ? (
                ...
                <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '20px', border: '1px solid var(--border)', marginBottom: '1.5rem' }}>
                  <div style={{ marginBottom: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                      {selectedJobForApply.requiredSkills?.filter(s => !selectedJobForApply.missingSkills.includes(s)).map(s => (
                        <span key={s} style={{ background: '#d1fae5', color: '#065f46', padding: '4px 10px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: '600' }}>{s}</span>
                      ))}
                      {selectedJobForApply.requiredSkills?.filter(s => !selectedJobForApply.missingSkills.includes(s)).length === 0 && <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>No matching requirements found.</span>}
                    </div>
                  </div>
          ...
          <div className="modal-content" style={{ maxWidth: '700px' }}>
            ...
            <div className="profile-grid">
              ...
              <div className="form-group">
                ...
                <input name="skills" value={profile.skills} onChange={handleProfileChange} placeholder="e.g. React, Python, SQL" />
              </div>
            </div>
            ...
            <div className="form-group" style={{ marginTop: '1rem' }}>
              ...
              <input id="res-up" type="file" style={{ display: 'none' }} onChange={(e) => handleFileChange(e, 'resume')} />
            </div>
                    ...
                    try {
                      ...
                      if (data.success && data.skills.length > 0) {
                        ...
                        const matchRes = await fetch('http://localhost:5000/api/opportunities/matched', {
                          method: 'POST',
                          headers: { 'Content-Type': 'application/json' },
                          body: JSON.stringify({ skills: data.skills, opportunities })
                        });
                        const matchData = await matchRes.json();
                        if (matchData.success) {
                          setMatchedJobs(matchData.matched);
