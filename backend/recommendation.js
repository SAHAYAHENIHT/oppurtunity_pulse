...
export function calculateMatchScore(opportunity, userProfile) {
  ...
  const searchText = (`${opportunity.title} ${opportunity.desc} ${opportunity.type} ${opportunity.organization}`).toLowerCase();

  // 1. Department & Course match (heavy weight)
  if (department && searchText.includes(department.toLowerCase())) score += 10;
  if (userProfile.courseEnrolled && searchText.includes(userProfile.courseEnrolled.toLowerCase())) score += 15;

  // 2. Type/Category Preferences
  if (preferences && Array.isArray(preferences) && preferences.length > 0) {
    if (preferences.includes(opportunity.type)) score += 30;
  }
  ...
  if (opportunity.sourceTrust === 'high') score += HIGH_TRUST_BONUS;
